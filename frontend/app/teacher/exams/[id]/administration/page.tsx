...
export default function ExamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 2. KONEKSI WEBSOCKET (DENGAN AUTO-RECONNECT & ENCODING) 📡
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      // [FIX] Gunakan encodeURIComponent untuk menangani spasi di username
      ...
      socket.onmessage = (event) => {
        try {
          ...
          console.log("PESAN DARI GURU:", data); // Debugging

          if (data.type === "command") {
            // RESET
            if (data.action === "reset_exam") {
              // Hapus timer DULUAN sebelum alert, biar gak lanjut hitung
